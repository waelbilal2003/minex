...
class _PostCardWidgetState extends State<PostCardWidget> {
  ...
  Timer? _refreshTimer; // <-- ✨ تم إضافة متغير للمؤقت
  ...
  void initState() {
    ...
    // --- ✨ بداية التعديل: تشغيل التحديث الدوري ---
    // سيتم تحديث الإحصائيات كل 15 ثانية
  ...
  @override
  void dispose() {
    // --- ✨ بداية التعديل: إيقاف المؤقت لمنع تسرب الذاكرة ---
    _refreshTimer?.cancel();
    // --- نهاية التعديل ---
    super.dispose();
  }

  // --- ✨ بداية التعديل: دالة جديدة لجلب الإحصائيات بشكل دوري ---
  Future<void> _refreshPostStats() async {
    // نتأكد أن الويدجت لا يزال موجودًا قبل إرسال الطلب
    if (!mounted) return;

    try {
      // نفترض وجود دالة جديدة في AuthService تجلب فقط الإحصائيات (أكثر كفاءة)
      // إذا لم تكن موجودة، سنقوم بإنشائها في الخطوة التالية
      final result = await AuthService.getPostStats(widget.post['id']);

      if (mounted && result['success'] == true) {
        // نقوم بالتحديث فقط إذا تغيرت البيانات
        if (result['likes_count'] != likesCount ||
            result['comments_count'] != commentsCount) {
          setState(() {
            likesCount = result['likes_count'] ?? likesCount;
            commentsCount = result['comments_count'] ?? commentsCount;
          });
        }
      }
    } catch (e) {
      // لا نعرض رسالة خطأ للمستخدم لأن هذا تحديث في الخلفية
      print("Periodic refresh failed for post ${widget.post['id']}: $e");
    }
  }
  // --- نهاية التعديل ---
              ...
              children: [
                _buildInteractionButton(
                  ...
                  onTap: () async {
                    ...
                    try {
                      final result =
                          await AuthService.togglePostLike(widget.post['id']);
                      if (result['success'] == true) {
                        if (mounted) {
                          setState(() {
                          ...
                          widget.post['likes_count'] = likesCount;
                        }
                      } else {
                        if (mounted) {
                          setState(() {
                ...
                _buildInteractionButton(
                  ...
                  onTap: () async {
                    ...
                    // إرسال رسالة أولية لبدء المحادثة
                    final result = await AuthService.sendMessage(
                      postUserId,
                      "مرحباً، بخصوص منشورك.",
                    );

                    // التأكد من أن الويدجت ما زال موجوداً قبل استخدام السياق
                    ...
                    if (result['success'] == true && result['data'] != null) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content:
                              Text(result['message'] ?? 'فشل في بدء المحادثة'),
                          backgroundColor: Colors.red,
                        ),
                      );
                    }
    ...
    try {
      ...
      if (result['success'] == true && mounted) {
        setState(() {
          ...
          widget.post['comments_count'] = commentsCount;
        });
      }
    } catch (e) {
