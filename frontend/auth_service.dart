...
class AuthService {
  ...
  // ✨ --- نهاية الإضافة --- ✨
  // ✨ دالة جديدة لجلب إحصائيات المنشور (الإعجابات والتعليقات)
  // جلب إحصائيات المنشور (الإعجابات والتعليقات)
  static Future<Map<String, dynamic>> getPostStats(int postId) async {
    try {
      final token = await getToken();
      if (token == null) {
      ...
      final response = await http
          .get(
            Uri.parse('$baseUrl/api/post-stats/$postId'),
            headers: getHeaders(token),
          )
          .timeout(const Duration(seconds: 15));

      return _handleResponse(response, 'get_post_stats');
    } catch (e) {
      return {'success': false, 'message': 'خطأ في جلب إحصائيات المنشور: $e'};
    }
  }
}
